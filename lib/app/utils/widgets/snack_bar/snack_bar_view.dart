...
import 'package:smart_attend/app/utils/widgets/snack_bar/snack_bar_view_controller.dart';

typedef OnTap = void Function(SnackBarView snack);

typedef SnackBarViewStatusCallback = void Function(SnackBarViewStatus? status);

class SnackBarView extends StatefulWidget {
  /// A callback for you to listen to the different Snack status
  final SnackBarViewStatusCallback? snackBarStatus;
  ...
  /// [snackPosition] == [SnackBarViewPosition.bottom] and [DismissDirection.up]
  /// when [snackPosition] == [SnackBarViewPosition.top]
  ...
  /// Give to SnackBar a gradient background.
  ...
  /// Snack can be based on [SnackBarViewPosition.top] or on [SnackBarViewPosition.bottom]
  /// of your screen.
  /// [SnackBarViewPosition.bottom] is the default.
  final SnackBarViewPosition snackPosition;
  ...
  const SnackBarView({
    ...
    this.snackPosition = SnackBarViewPosition.bottom,
    this.snackStyle = SnackBarViewStyle.floating,
  ...
  @override
  State createState() => SnackBarViewState();

  /// Show the snack. It's call [SnackBarViewStatus.opening] state
  /// followed by [SnackBarViewStatus.open]
  SnackBarViewController show() {
    return showSnackBarView(this);
  }

  SnackBarViewController showSnackBarView(SnackBarView snackBar) {
    final controller = SnackBarViewController(snackBar);
    controller.show();
    return controller;
  }
}
...
class SnackBarViewState extends State<SnackBarView>
  ...
  Widget build(BuildContext context) {
    return Align(
      heightFactor: 1.0,
      child: Material(
        color: widget.snackStyle == SnackBarViewStyle.floating
            ? Colors.transparent
            : widget.backgroundColor,
        child: SafeArea(
          minimum: widget.snackPosition == SnackBarViewPosition.bottom
              ? EdgeInsets.only(
                  bottom: MediaQuery.of(context).viewInsets.bottom)
              : EdgeInsets.only(top: MediaQuery.of(context).padding.top),
          bottom: widget.snackPosition == SnackBarViewPosition.bottom,
          top: widget.snackPosition == SnackBarViewPosition.top,
          left: false,
          right: false,
          child: Stack(
            children: [
...
You need to either use message[String], or messageText[Widget] or define a userInputForm[Form] in SnackBar''');
    ...
    _isMessagePresent = (widget.message != null || widget.messageText != null);
                  ...
                  children: <Widget>[
                    _isTitlePresent
                        ? Padding(
                            ...
                            child: widget.titleText ?? _emptyWidget,
                          )
                        : _emptyWidget,
                    _isMessagePresent
                        ? Padding(
                            ...
                            child: widget.messageText ?? _emptyWidget,
                          )
                        : _emptyWidget,
                  ],
                ),
...
enum SnackBarViewStatus { open, closed, opening, closing }

enum SnackBarViewPosition { top, bottom }

enum SnackBarViewStyle { floating, grounded }
